...
class PreviousActivitiesPage {
  ...
  setExpenditures(expenditures) {
    for (let i = 0; i < this.years.length; i += 1) {
      cy.get(`[name='hithie-approved-total-${this.years[i]}']`).type(
        expenditures.hithie.approved[i]
      );
      cy.get(`[name='hithie-actual-federal-${this.years[i]}']`).type(
        expenditures.hithie.actual[i]
      );

      cy.get(`[name='approved-total-mmis90-${this.years[i]}']`).type(
        expenditures.mmis90.approved[i]
      );
      cy.get(`[name='actual-federal-mmis90-${this.years[i]}']`).type(
        expenditures.mmis90.actual[i]
      );

      cy.get(`[name='approved-total-mmis75-${this.years[i]}']`).type(
        expenditures.mmis75.approved[i]
      );
      cy.get(`[name='actual-federal-mmis75-${this.years[i]}']`).type(
        expenditures.mmis75.actual[i]
      );

      cy.get(`[name='approved-total-mmis50-${this.years[i]}']`).type(
        expenditures.mmis50.approved[i]
      );
      cy.get(`[name='actual-federal-mmis50-${this.years[i]}']`).type(
    ...
    for (let i = 0; i < this.years.length; i += 1) {
      ...
      cy.get(`[name='hithie-approved-total-${this.years[i]}']`)
        ...
        .then(val => {
          ...
          cy.get(
            `[headers="prev_act_hithie_row_${this.years[i]} ` +
      ...
      cy.get(`[name='approved-total-mmis90-${this.years[i]}']`)
        ...
        .then(val => {
          ...
          // Get the ith year's calculated FFP
          cy.get(
            `[headers="prev_act_mmis_row_${this.years[i]} ` +
  ...
  verifyTotalFFP() {
    ...
    for (let i = 0; i < this.years.length; i += 1) {
      cy.get(
        `[headers="prev_act_total_row_${this.years[i]} prev_act_total_approved"]`
      )
        .invoke('text')
        .then(text => {
  ...
  verifyTotalExpenditures() {
    ...
    for (let i = 0; i < this.years.length; i += 1) {
      cy.get(
        `[headers="prev_act_total_row_${this.years[i]} prev_act_total_actual"`
      )
        .invoke('text')
        .then(text => {
          const yearlyTotalExpenditure = extractNumber(text);
          cy.log({ yearlyTotalExpenditure });
          cy.log('total', totals[this.years[i]]);
