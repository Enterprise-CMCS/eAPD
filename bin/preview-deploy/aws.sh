...
function configureUserData() {
  ...
  sed -i'.backup' -e "s|__PREVIEW_MONGO_INITDB_ROOT_USERNAME__|`echo $PREVIEW_MONGO_INITDB_ROOT_USERNAME`|g" aws.user-data.sh
  
  sed -i'.backup' -e "s|__PREVIEW_MONGO_INITDB_ROOT_PASSWORD__|`echo $PREVIEW_MONGO_INITDB_ROOT_PASSWORD`|g" aws.user-data.sh

  sed -i'.backup' -e "s|__PREVIEW_MONGO_INITDB_DATABASE__|`echo $PREVIEW_MONGO_INITDB_DATABASE`|g" aws.user-data.sh

  sed -i'.backup' -e "s|__PREVIEW_MONGO_DATABASE_USERNAME__|`echo $PREVIEW_MONGO_DATABASE_USERNAME`|g" aws.user-data.sh

  sed -i'.backup' -e "s|__PREVIEW_MONGO_DATABASE_PASSWORD__|`echo $PREVIEW_MONGO_DATABASE_PASSWORD`|g" aws.user-data.sh
