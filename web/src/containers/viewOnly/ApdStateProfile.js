...
const ApdStateProfile = ({stateProfile, keyPersonnel}) => {
  const totalCost = (person) => Object.values(person.costs).reduce((sum, value) => sum + +value, 0);
  const costByYear = (person) => Object.keys(person.costs).map(year => (
          <li key={year}>
            <strong>FFY {year} cost:</strong> <Dollars>{person.costs[year]}</Dollars>
          </li>
        ));
    ...
    return (
      <Fragment>
        <ul className="ds-c-list--bare">
          <li><h3>{index + 1}. {person.name}</h3></li>
          {person.isPrimary ? <li>Primary APD Point of Contact</li> : null}
          <li>{person.position}</li>
          <li><strong>Email: </strong>{person.email}</li>
          <li><strong>Percent time allocated to project: </strong>{person.percentTime}%</li>
          <li>
            <strong>Total costs: </strong><Dollars>{person.hasCosts ? totalCost(person) : '0' }</Dollars>
  ...
  return (
    <div>
      ...
      <addr>
          {stateProfile.medicaidOffice.address1}<br />
          {stateProfile.medicaidOffice.address2 &&
              stateProfile.medicaidOffice.address2 }
          {stateProfile.medicaidOffice.city}, {stateProfile.medicaidOffice.state} {stateProfile.medicaidOffice.zip}
      </addr>
