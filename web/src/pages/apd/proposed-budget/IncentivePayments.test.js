...
describe('<IncentivePayments />', () =>{
  ...
  test('renders correctly', async () => {
    ...
    data.forEach((d) => {
      ...
      quarters.forEach((q) => {
        ...
        expect(screen.getByTestId(`${name} ${firstYear} ${q}`)).toHaveValue(`${firstValues[index]}`);
        expect(screen.getByTestId(`${name} ${secondYear} ${q}`)).toHaveValue(`${secondValues[index]}`);
      })
    })
  });
  ...
  test('renders incentive payments subtotal', async () => {
    ...
    var ehAmtYearOneTotal = [1, 2, 3, 4].reduce((partialSum, a) => partialSum + a, 0),
        ehAmtYearTwoTotal = [5, 6, 7, 8].reduce((partialSum, a) => partialSum + a, 0),
        epAmtYearOneTotal = [17, 18, 19, 20].reduce((partialSum, a) => partialSum + a, 0),
        epAmtYearTwoTotal = [21, 22, 23, 24].reduce((partialSum, a) => partialSum + a, 0);

    waitFor(() => {
      expect(screen.getByTestId('ehAmt 2022 total').innerHTML.replace(/<(.|\n)*?>/g, '')).toHaveValue(`$${ehAmtYearOneTotal}`);
    }).catch(err => console.log(err));

    waitFor(() => {
      expect(screen.getByTestId('epAmt 2022 total').innerHTML.replace(/<(.|\n)*?>/g, '')).toHaveValue(`$${epAmtYearOneTotal}`);
    }).catch(err => console.log(err));

    waitFor(() => {
      expect(screen.getByTestId('ehAmt 2023 total').innerHTML.replace(/<(.|\n)*?>/g, '')).toHaveValue(`$${ehAmtYearTwoTotal}`);
    }).catch(err => console.log(err));

    waitFor(() => {
      expect(screen.getByTestId('epAmt 2023 total').innerHTML.replace(/<(.|\n)*?>/g, '')).toHaveValue(`$${epAmtYearTwoTotal}`);
    }).catch(err => console.log(err));
  });
});
