...
describe('Filling out Key Personnel for eAPD with valid login token', () => {
  ...
  describe('Add blank key personnel', () => {
    ...
    before(() => {
      ...
      cy.get('.ds-c-vertical-nav__label--parent').contains('Key State Personnel').click();

      // Click on nav submenu button
      cy.get('a.ds-c-vertical-nav__label').contains("Key State Personnel").click();
    ...
    it('Verify no primary contacts message', () => {
      cy.contains('Primary Point of Contact has not been added for this activity').should('exist');
    });
    ...
    it('Add blank primary contact', () => {
      ...
      cy.get('@primaryContactVals')
        .findByRole('heading', { name: /Primary Point of Contact name not specified/i })
        .should('exist');
      cy.get('@primaryContactVals')
        .find('li')
        .should(($lis) => {
    ...
    it('Add blank key personnel that is chargeable to the project', () => {
      ...
      cy.get('input[type="radio"][value="yes"]').scrollIntoView().check({ force: true });
      cy.findByRole('button', { name: /Done/i }).click();
      ...
      for(let year of years) {
        cy.get('@personnelVals')
          .should('contain', 'FFY ' + year + ' Cost: $0 | FTE: 0 | Total: $0')
      }
    });
  });
  ...
  describe('Verify export data for blank key personnel', () => {
    ...
    it('The 3 blank key personnel have default values', () => {
      ...
      cy.get('@personnel')
        .findByRole('heading', { name: /Key Personnel and Program Management/i })
        .next()
        .find('ul')
        .first()
        .should('have.text', 
        ...
        let str = '3. Key Personnel name not specified' +
        'Role not specified' +
        'Email: ';
        for(let year of years) {
  ...
  describe('Fill out Key State Personnel Page', () => {
    ...
    it('Fill out State Director and Office Address', () => {
      cy.fixture('users').then((userData) => {
        ...
        cy.get('input[name="apd-state-profile-mdemail"]').type(userData[0].email);
        cy.get('input[name="apd-state-profile-mdphone"]').type(userData[0].phone);
        cy.get('input[name="apd-state-profile-addr1"]').type(userData[0].address.street);
        cy.get('input[name="apd-state-profile-addr2"]').type(userData[0].address.suite);
        cy.get('input[name="apd-state-profile-city"]').type(userData[0].address.city);
    ...
    describe('Fill out Primary Point of Contact', () => {
      ...
      it('Edit Primary Point of Contact', () => {
        ...
        cy.fixture('users').then((userData) => {
          cy.get('input[name="apd-state-profile-pocname0"]').type(userData[1].name);
          cy.get('input[name="apd-state-profile-pocemail0"]').type(userData[1].email);
    ...
    describe('Fill out other Key Personnel', () => {
      ...
      it('Edit Key Personnel', () => {
        ...
        cy.fixture('users').then((userData) => {
          cy.get('input[name="apd-state-profile-pocname1"]').type(userData[2].name);
    ...
    it('Pressing continue navigates to Results of Previous Activities page', () => {
      ...
      cy.findByRole('heading', { name: /Results of Previous Activities/i }).should('exist');
    });
    ...
    it('Medicaid director has input values', () => {
      cy.fixture('users').then((userData) => {
        cy.get('@personnel')
          ...
          .next()
          .should('have.text', 
            'Name:  ' + userData[0].name +    // Mysterious extra space
    ...
    it('Medicaid office address has input values', () => {
      cy.fixture('users').then((userData) => {
        ...
        cy.get('@personnel')
          ...
          .next()
          .should('have.text', 
            userData[0].address.street + userData[0].address.suite + 
            userData[0].address.city + ', AL ' +
