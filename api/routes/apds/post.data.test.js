...
tap.test('APD data initializer', async apdTests => {
  ...
  apdTests.test('create HITECH APD with years', async test => {
    const output = await getNewApd(APD_TYPE.HITECH, ['1998', '1999']);
  ...
  apdTests.test('create MMIS APD with years', async test => {
    const output = await getNewApd(APD_TYPE.MMIS, ['1998', '1999']);
