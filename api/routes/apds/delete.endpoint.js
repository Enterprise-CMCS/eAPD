  ...
  describe('Delete/archive APD endpoint | DELETE /apds/:id', () => {
    ...
    describe('when authenticated as a user with permission', () => {
      it('with a non-existant apd ID', async () => {
        const response = await login().then(api => api.delete(url(9000)));
      ...
      it(`with an APD in a state other than the user's state`, async () => {
        const response = await login().then(api => api.delete(url(4001)));
      ...
      it('with an APD that is not in draft', async () => {
        const response = await login().then(api => api.delete(url(4002)));
