exports.up = async knex => {
  await knex.schema.table('state_admin_certifications', table => {
    ...
    table.integer('ffy').notNullable().comment('FFY that the letter is valid for');
    table.string('name').notNullable().comment('full name of the individual being certified');
    table.string('email').notNullable().comment('email of the individual being certified');
    table.string('phone').notNullable().comment('phone number of the individual being certified');
    ...
    table.date('uploadedOn').notNullable().comment('date certification letter was initiall added');
        
    table.integer('affiliationId').comment('id of the auth_affiliation this certification is tied to');
    table.foreign('affiliationId')
      .references('auth_affiliations.id');
  ...
  await knex.schema.table('state_admin_certifications_audit', table => {
    ...
    table.integer('certificationId').comment('id of the state_admin_certifications record');
    table.foreign('certificationId')
