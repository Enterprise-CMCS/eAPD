...
tap.test('database wrappers / oktaUsers', async oktaUsersTests => {
  ...
  oktaUsersTests.test('creating an Okta User', async test => {
    db.insert.withArgs({user_id: 'oktaUser', email: 'email', metadata: 'metadata'}).resolves('oktaUser id');

    const newUser = await createOktaUser('oktaUser', 'email', 'metadata' ,{ db });
  ...
  oktaUsersTests.test('updating an Okta User', async test => {
    ...
    db.update.withArgs({ email: 'email', metadata: 'metadata'}).resolves('updated okta_user');

    const updatedUser = await updateOktaUser('oktaUser', 'email', 'metadata' ,{ db });
  ...
  oktaUsersTests.test('createOrUpdate creating an Okta User', async test => {
    ...
    const updatedUser = await createOrUpdateOktaUser('oktaUser', 'email', 'metadata' ,{ db });
  ...
  oktaUsersTests.test('createOrUpdateOktaUserFromOkta creating oktaUser', async test => {
    ...
    const createdUser = await createOrUpdateOktaUserFromOkta('oktaUser' ,{ okta, db });
  ...
  oktaUsersTests.test('createOrUpdateOktaUserFromOkta updating oktaUser', async test => {
    const okta = sinon.stub()
    okta.getUser = () => {
      return {profile: toSanitize}
    }
    ...
    const updatedUser = await createOrUpdateOktaUserFromOkta('oktaUser' ,{ okta, db });
